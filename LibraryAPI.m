...
@interface LibraryAPI () <AVAudioPlayerDelegate> {
...
- (NSArray*)getSongs
{
    return [persistenceManager getSongs];
}

- (void)addSong:(Song*)song atIndex:(int)index
{
    [persistenceManager addSong:song atIndex:index];
    if (isOnline)
    {
...
{
    [persistenceManager deleteSongAtIndex:index];
    if (isOnline)
    {
...
- (void)configureAudioPlayer {
    ...
    NSString *backgroundMusicPath = [[NSBundle mainBundle] pathForResource:@"achievement" ofType:@"wav"];
...
- (void)configureSystemSoundName:(NSString*)name ofType:(NSString*)type  {
    ...
    NSString *pewPewPath = [[NSBundle mainBundle] pathForResource:name ofType:type];
...
//- (void)playSong:(Song*)song atIndex:(int)index{
//    
